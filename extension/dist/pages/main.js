...
function accountItem_drag(ev) {
    ...
    if (!draggingEl) {
        ...
        if (draggingEl)
            draggingEl.classList.add("invisible");
        new d.All("li.account-item").toggleClass("unselectable");
    }
}
function accountItem_dragOver(ev) {
...
//   }
// }
function accountItem_drop(ev) {
    ev.preventDefault();
    //console.log("drop")
}
async function accountItem_dragend(ev) {
    //console.log("dragEnd")
    ev.preventDefault();
...
function sortByOrder(a, b) {
    if (a.accountInfo.order > b.accountInfo.order)
        return 1;
    return -1;
}
...
//---------------------------------------------------
export function accountItemClicked(ev) {
