...
import { assetIsStakingPool, ExtendedAccountData, StakingPoolAsset, } from "../data/account.js";
...
function showSelectedAccount() {
    ...
    if (assetIsStakingPool(selectedAccountData.accountInfo.assets[0])) {
...
async function checkAccountAccess() {
    if (selectedAccountData.accountInfo.type == "lock.c") {
        ...
        const ownerInfo = await getAccountRecord(selectedAccountData.accountInfo.ownerId);
        if (!ownerInfo)
...
async function fullAccessSubPage(subPageId, OKHandler) {
    try {
        d.hideErr();
...
//--------------------------------
async function checkOwnerAccessThrows(action) {
...
//       d.showErr(ex.message);
//     }
//     let actualSP = selectedAccountData.accountInfo.assets;
//     let poolAccInfo = {
//       //empty info
//       account_id: "",
//       unstaked_balance: "0",
...
//---------------------------------------------
export async function searchThePools(exAccData) {
                ...
                if (isStakingPool && poolAccInfo) {
                    ...
                    if (amount > 0) {
                        ...
                        if (amount > lastAmountFound) {
                            ...
                            stakingPool.stakingPool = pool.account_id;
                            stakingPool.staked = c.yton(poolAccInfo.staked_balance);
                            stakingPool.unstaked = c.yton(poolAccInfo.unstaked_balance);
                            exAccData.inThePool = stakingPool.staked + stakingPool.unstaked;
                            stakingPool.stakingPoolPct = await StakingPool.getFee(pool.account_id);
                            exAccData.accountInfo.assets.push(stakingPool);
...
async function AccountDeleteOKClicked() {
    if (!selectedAccountData || !selectedAccountData.accountInfo)
        return;
    try {
        d.showWait();
        if (selectedAccountData.isReadOnly)
