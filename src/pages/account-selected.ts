...
function showSelectedAccount() {
  ...
  if (selectedAccountData.accountInfo.lockedOther) {
    const lockedOthLine = new d.El(
      ".selected-account-info #locked-others-line"
    );
    lockedOthLine.show();
  }
  ...
  d.onClickSelector(
    ".selected-account-info .access-status",
    accessLabelClicked
  );
}
        ...
        if (isStakingPool && poolAccInfo) {
          ...
          if (amount > 0) {
            ...
            if (amount > lastAmountFound) {
              ...
              stakingPool.stakingPoolPct = await StakingPool.getFee(
                pool.account_id
              );
