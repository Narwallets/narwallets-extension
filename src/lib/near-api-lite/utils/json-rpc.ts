  ...
  if (largeNumbersFound) {
    for (const matches of largeNumbersFound) {
      ...
      if (yoctosString.length >= 20) {
        // convert to NEAR
        text = text.replace(
          new RegExp(yoctosString, "g"),
          ytonFull(yoctosString)
        );
      }
    }
  }
...
let id = 0;
export async function jsonRpcInternal(
  ...
  try {
    ...
    while (true) {
      ...
      if (!fetchResult.ok)
        throw new Error(
          rpcUrl + " " + fetchResult.status + " " + fetchResult.statusText
        );
 ...
 */
export async function jsonRpcQuery(
  queryWhat: string,
  params?: any
): Promise<any> {
